...
clientes = []
reservas = []
nao_autorizados = []  # Lista para hóspedes menores de 18 anos

if reserva.lower() == 's':
    # Quantidade de hóspedes
    quantidade_hospede = int(input('Quantos Resevas seram  feitas? '))
    
    # Loop para processar múltiplos hóspedes para fazer suas reservas
    for n in range(quantidade_hospede):
        print(f'\nCadastro do Hóspede {n + 1}')
        nome = input('Por favor, digite seu nome: ')
        idade = int(input("Qual a sua idade: "))

        if idade < 18:
            print('Você não pode reservar um quarto, chame um responsável, por favor.')
            nao_autorizados.append(nome)  # Adiciona à lista de não autorizados
            continue  # Passa para o próximo hóspede
        ...
        if quarto in quartos:
            ...
            reservas.append((nome, nome_quarto, dias, total))
            print(f'{nome}, você escolheu o quarto {nome_quarto} e ficará hospedado por {dias} dias, pagando o valor total de R${total}.')
        else:
...
# Resumo final
if reservas or nao_autorizados:
    print('\nResumo das Reservas:')
    
    # Mostrando reservas válidas
    for reserva in reservas:
        nome, quarto, dias, total = reserva
        print(f'{nome} escolheu o quarto {quarto} por {dias} dias, totalizando R${total:.2f}.')
    
    # Mostrando hóspedes não autorizados
    if nao_autorizados:
        print('\nHóspedes que não puderam reservar:')
        for nome in nao_autorizados:
            print(f'{nome} não conseguiu fazer a reserva. Precisa chamar um responsável.')
else:
    print('Nenhuma reserva foi concluída.')
